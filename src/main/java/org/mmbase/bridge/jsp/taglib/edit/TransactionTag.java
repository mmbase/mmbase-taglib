...
public class TransactionTag extends CloudReferrerTag implements CloudProvider {
    ...
    protected void refreshTransaction() throws JspTagException {
        ...
        if (transaction == null) {
            Cloud c = super.getCloudVar();
            if (c instanceof Transaction && ((Transaction) c).isCommitted()) {
                c = c.getNonTransactionalCloud();
            }
            transaction = c.getTransaction(n);
        } else {
